  ...
  <div class="dash-main">
    ...
    <div class="dash-content">
      ...
      <div class="section-card" style="margin-bottom:1.5rem;">
        <div class="section-card-header">
          <h2><?= icon('users') ?> Schema: <code style="font-size:0.9em;">users</code></h2>
          <span style="font-size:0.78rem;color:var(--text-muted);"><?= $total_users_count ?> rows</span>
        </div>
        <div style="overflow-x:auto;">
          <table class="col-table">
            <thead>
            ...
            <tbody>
              <?php foreach ($users_cols as $col): ?>
              <tr>
                <td style="font-weight:600;color:var(--text);font-family:monospace;"><?= e($col['Field']) ?></td>
                <td class="db-mono"><?= e($col['Type']) ?></td>
                <td style="font-size:0.8rem;"><?= $col['Null']==='YES' ? '<span style="color:var(--text-dim)">YES</span>' : '<span style="color:var(--success)">NO</span>' ?></td>
                <td>
                  <?php if ($col['Key']==='PRI'): ?><span class="key-badge key-pri">PRI</span>
                  <?php elseif ($col['Key']==='UNI'): ?><span class="key-badge key-uni">UNI</span>
                  <?php elseif ($col['Key']==='MUL'): ?><span class="key-badge key-mul">MUL</span>
                  <?php endif; ?>
                </td>
                <td class="db-mono"><?= e($col['Default'] ?? 'NULL') ?></td>
                <td class="db-mono" style="font-size:0.75rem;"><?= e($col['Extra']) ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
      ...
      <div class="section-card" style="margin-bottom:1.5rem;">
        <div class="section-card-header">
          <h2><?= icon('activity') ?> Schema: <code style="font-size:0.9em;">session_log</code></h2>
          <span style="font-size:0.78rem;color:var(--text-muted);"><?= $total_session_count ?> rows</span>
        </div>
        <div style="overflow-x:auto;">
          <table class="col-table">
            <thead>
            ...
            <tbody>
              <?php foreach ($session_cols as $col): ?>
              <tr>
                <td style="font-weight:600;color:var(--text);font-family:monospace;"><?= e($col['Field']) ?></td>
                <td class="db-mono"><?= e($col['Type']) ?></td>
                <td style="font-size:0.8rem;"><?= $col['Null']==='YES' ? '<span style="color:var(--text-dim)">YES</span>' : '<span style="color:var(--success)">NO</span>' ?></td>
